...
function estimatePrint($estimates, $o = array()) {
  ...
  $html .= '<tr style="background: #DDEEEE"><td><b>Phase/Detail</b></td><td><b>Low</b></td><td><b>High</b></td><td><b>Category</b></td></tr>';
   foreach($estimates as $e) {
    $html .= '<tr><td><b>' . $e->getName() . '</b><br />';
    $html .= $e->getData('notes') . '</td>';
    $html .= '<td>' . $e->getLowEstimate() . '</td>';
    $html .= '<td>' . $e->getHighEstimate() . '</td>';
    $html .= '<td>' . $e->get('category') . '</td></tr>';
  }
  $html .= '</table>';
  return $html;
}
