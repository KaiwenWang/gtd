...
class SupportContractController extends PageController {
  ...
  function renew($params) {
    $d = $this->data;
    $params['id']
      ? $d->old_contract = new SupportContract($params['id'])
      : Bail('required parameter $params["id"] missing.');

    $d->new_contract = $d->old_contract->cloneRecord();

    strtotime($d->old_contract->get('end_date')) > time() 
      ? $end_date = $d->old_contract->get('end_date')
      : $end_date = '';
  ...
  function cancel($params) {
    $d = $this->data;
    $params['id']
      ? $d->contract = new SupportContract($params['id'])
      : Bail('required parameter $params["id"] missing.');

    if($d->contract->get('end_date') == EMPTY_DATE_STRING) {
      $d->contract->set(array('end_date' => date('Y-m-d')));
