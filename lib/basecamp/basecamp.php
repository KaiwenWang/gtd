...
class Basecamp {
  ...
  function todo_items($item_id) {
    return $this->hook("/todo_items/{$item_id}","item");
  }
  ...
  function update_milestone($milestone_id,$milestone,$move_upcoming_milestones = true,$move_upcoming_milestones_off_weekends = true) {
   ...
   function time_entries($project_id) {
    return $this->hook("/projects/{$project_id}/time_entries.xml");
  } 
  /*===================/
   *   The Worker Bees
  /*===================*/
...
#$projects = $session->todo_items('31767698');
#$projects = $session->time_entries('2440460');
#AMP_dump($projects);
